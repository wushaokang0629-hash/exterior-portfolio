...
export default function RingHome(){
 ...
 useEffect(()=>{
  ...
  let radius=420,frame=0,last=0,running=!intro;
  const size=()=>{const w=host.clientWidth,c=w<600?Math.min(240,w*.60):Math.min(300,w*.205);radius=c/(2*Math.tan(Math.PI/projects.length))+(w<600?24:36);host.style.setProperty('--ring-radius',`${radius}px`);host.style.setProperty('--ring-card-width',`${c}px`);};
  ...
  const context=gsap.context(()=>{
   if(intro){gsap.set(camera.current,{rotateX:-78,scale:.62,y:-85});gsap.set('.ring-home-copy, .ring-home-nav, .ring-home-footer, .ring-controls',{opacity:0,y:12});gsap.timeline({onComplete:reveal}).to(camera.current,{rotateX:-20,scale:1.06,y:-20,duration:1.55,ease:'power3.inOut'},.15).to(camera.current,{rotateX:0,scale:1,y:0,duration:.95,ease:'power3.out'},1.55).to('.ring-home-copy, .ring-home-nav, .ring-home-footer, .ring-controls',{opacity:1,y:0,stagger:.08,duration:.65},1.85);}else reveal();
  },host);
  const render=time=>{const dt=Math.min((time-(last||time))/1000,.05);last=time;const m=motion.current;if(running&&!reduced&&!m.paused&&!m.dragging&&!document.hidden)m.target-=dt*4;m.angle+=(m.target-m.angle)*(1-Math.exp(-dt*9));ring.current.style.transform=`translateZ(${-radius}px) rotateY(${m.angle}deg)`;cards.current.forEach((card,i)=>{if(!card)return;const facing=Math.cos((i*step+m.angle)*Math.PI/180);card.style.opacity=String(.12+.88*Math.max(0,facing));card.style.filter=`brightness(${.48+.52*Math.max(0,facing)})`;card.style.pointerEvents=running&&facing>.12?'auto':'none';card.tabIndex=running&&facing>.12?0:-1;});const index=((Math.round(-m.angle/step)%projects.length)+projects.length)%projects.length;setActive(p=>p===index?p:index);frame=requestAnimationFrame(render);};
  frame=requestAnimationFrame(render);return()=>{cancelAnimationFrame(frame);observer.disconnect();context.revert();};
 },[step]);
 ...
 return <main className="ring-home" ref={root}>
  ...
  <section className="ring-stage" aria-label="精选作品环形画廊" onPointerEnter={()=>{motion.current.paused=true;}} onPointerLeave={()=>{motion.current.paused=false;motion.current.dragging=false;}} onFocusCapture={()=>{motion.current.paused=true;}} onBlurCapture={e=>{if(!e.currentTarget.contains(e.relatedTarget))motion.current.paused=false;}}
   onPointerDown={e=>{if(!ready||e.button!==0)return;Object.assign(motion.current,{dragging:true,lastX:e.clientX,distance:0});}}
   ...
   <div className="ring-camera" ref={camera}><div className="project-ring" ref={ring}>{projects.map((p,i)=><a className="ring-card" key={p.id} ref={el=>{cards.current[i]=el;}} href={`#/works/${p.category}/${p.id}`} style={{transform:`rotateY(${i*step}deg) translateZ(var(--ring-radius))`}} aria-label={`查看${p.title}`} draggable="false"><img src={p.image} alt={p.title} draggable="false" fetchPriority={i===0?'high':'auto'}/><div className="ring-card-caption"><span>{categories.find(c=>c.id===p.category)?.name}</span><h2>{p.title}</h2></div></a>)}</div></div>
  </section>
  <div className="ring-controls"><button type="button" onClick={()=>move(1)} aria-label="上一个精选项目" disabled={!ready}>←</button><span>{String(active+1).padStart(2,'0')}<i>/</i>{String(projects.length).padStart(2,'0')}</span><button type="button" onClick={()=>move(-1)} aria-label="下一个精选项目" disabled={!ready}>→</button></div>
